    ...
    const transformedEduData = useMemo(
        () =>
            Array.isArray(eduData)
                ? eduData
                : Object.values(eduData).flatMap((item) => (Array.isArray(item) ? item : Object.values(item))),
        [eduData],
    );
    ...
    const transformedExpData = useMemo(
        () =>
            Array.isArray(expData)
                ? expData
                : Object.values(expData).flatMap((item) => (Array.isArray(item) ? item : Object.values(item))),
        [expData],
    );
